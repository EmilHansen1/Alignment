...
jmax = 40
j0 = 0
jl1 = np.arange(j0, jmax, 2)
jl2 = np.arange(j0+1, jmax, 2)

wgnj1 = np.zeros((jl1.shape[0],2*jl1[-1]+1,6))
wgnj2 = np.zeros((jl2.shape[0],2*jl2[-1]+1,6))
for ij in range(len(jl1)):
    for mli in range(-jl1[ij],jl1[ij]+1):
        if (np.abs(mli)<=jl1[ij]-2):
            wgnj1[ij,mli+jl1[ij],0] = wigner_3j(jl1[ij], jl1[ij]-2, 2, mli, -mli, 0)
        wgnj1[ij,mli+jl1[ij],1] = wigner_3j(jl1[ij], jl1[ij]-2, 2, mli, -(mli+2), 2)
        wgnj1[ij,mli+jl1[ij],2] = wigner_3j(jl1[ij], jl1[ij], 2, mli, -mli, 0)
        wgnj1[ij,mli+jl1[ij],3] = wigner_3j(jl1[ij], jl1[ij], 2, mli, -(mli+2), 2)
        wgnj1[ij,mli+jl1[ij],4] = wigner_3j(jl1[ij], jl1[ij]+2, 2, mli, -mli, 0)
        wgnj1[ij,mli+jl1[ij],5] = wigner_3j(jl1[ij], jl1[ij]+2, 2, mli, -(mli+2), 2)
    for mli in range(-jl2[ij],jl2[ij]+1):
        if (np.abs(mli)<=jl2[ij]-2):
            wgnj2[ij,mli+jl2[ij],0] = wigner_3j(jl2[ij], jl2[ij]-2, 2, mli, -mli, 0)
        wgnj2[ij,mli+jl2[ij],1] = wigner_3j(jl2[ij], jl2[ij]-2, 2, mli, -(mli+2), 2)
        wgnj2[ij,mli+jl2[ij],2] = wigner_3j(jl2[ij], jl2[ij], 2, mli, -mli, 0)
        wgnj2[ij,mli+jl2[ij],3] = wigner_3j(jl2[ij], jl2[ij], 2, mli, -(mli+2), 2)
        wgnj2[ij,mli+jl2[ij],4] = wigner_3j(jl2[ij], jl2[ij]+2, 2, mli, -mli, 0)
        wgnj2[ij,mli+jl2[ij],5] = wigner_3j(jl2[ij], jl2[ij]+2, 2, mli, -(mli+2), 2)'''
...
def VNa2(theta):
    ac = 2.30930382013488
    bc = 0.489642923178541
    cc = 0.162304514806892
    dc = 0.219465539905037
    cth = np.cos(theta)
    return -((ac - (ac*(3 + ac) - 6*bc)*cth**2 + (ac**2 - 10*bc - 4*ac*bc + 15*cc)*cth**4 + 
            (4*(ac - bc)*bc - 21*cc - 6*ac*cc + 28*dc)*cth**6 + 2*(2*bc*(bc - 3*cc) + 3*ac*cc - 2*(9 + 2*ac)*dc)*
            cth**8 + (-9*cc**2 + 4*bc*(3*cc - 4*dc) + 8*ac*dc)*cth**10 + (9*cc**2 + 16*bc*dc - 24*cc*dc)*
              cth**12 + 8*(3*cc - 2*dc)*dc*cth**14 + 16*dc**2*cth**16))
...
I0 = 6.7*1e-2                   # Laser intensity
B = 4.53 * 2 * np.pi *1e-3      # B constant
Bg = 4.638 * 2 * np.pi *1e-3    # Related to V_He
...
if __name__ == '__main__':
    ...
    pf.to_csv("Na3DSingletWithUInhomogenousB4.53D0.08I3.53e10WFA.csv")
