...
# Create the operators for observable and interaction
...
def IntCosSq(jp, mp, j, m):
    # Integral ogf two spherical harmonics for phi = 0
...
def ftD(ti, t, ct, dt, step, idx, Bk, Bp, I0, t0, tau, dal, alp, w0, v0):
    ...
    res[0] = ct # Initial state
    for it, tl in enumerate(trange[1:]):
        if (np.abs(tl - t0) > 1.5 * tau):
            ctc = np.exp(-w0*dt*1j)*ct
        else:
            Il = It(I0, t0, tau, tl)
            Hmat = Bk*np.diag(Hkin[idx])+Bp*cosVmatF[idx]-Il * alp * np.eye(Hkin[idx].shape[0]) - Il * dal * sincosmat[idx] # Entire Hamiltonian
        ...
        ct = ctc.copy()
    return trange[::step],res   # res[i] = phi(t_i)

def getCosNa2(Blk, Blp,  Il):
    print(Blk, "\t", Blp)
    # Create and diagonalize the field-free Hamiltonians
    ...
    def getCosj(idx, stind):
        ctl = np.zeros(v[idx].shape[0]) # Wavefunction
        ctl[stind] = 1                  # Set current (|jm> or |n>?) state to be propagated
        cosMat = np.dot(np.conj(v[idx].T),np.dot(cosSqmatF[idx], v[idx]))   # Get cosSqmatF in the basis
        ctr = ftD(ti, tmax, ctl, dt, step, idx, Blk, Blp, Il, t0, tau, dal, alp, w[idx], v[idx])
        zd1 = np.dot(cosMat, ctr[1].T)  # Multiply cos2 on each of the states for all times
        z = np.real(np.array([np.dot(np.conj(ctr[1][il]), zd1[:,il]) for il in range(ctr[1].shape[0])]))
        return np.real(z)

    thresh = 1e-4
    we = np.sort(np.concatenate((w0, w1)))  # Sorted even eigenvalues
    wo = np.sort(np.concatenate((w2, w3)))  # Sorted odd eigenvalues
    me = 5/8
    mo = 3/8
...
I0 = 3.53*1e-2                  # Intensity
B = 4.53 * 2 * np.pi *1e-3      
Bg = 4.638 * 2 * np.pi *1e-3
cons = 2 * np.pi / (3 * 1.05)
Ir = cons * I0
ti = -20.0
t0 = 0.0
...
def getCosIBL(Bl):
    # Calculate trace for a singlet B-value
...
if __name__ == '__main__':
    ...
    pf.to_csv('NaSingletTestNew.csv')
    #pf.to_csv("Na3DSingletWithUInhomogenousB4.53D0.19I3.53e10WFA.csv")
