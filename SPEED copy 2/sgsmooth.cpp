...
static float_vect sg_derivative(const float_vect &v, const int w, 
                                const int deg, const double h=1.0);

// c-callable interface to for tcl plugin code
extern "C" double *calc_sgsmooth(const int ndat, double *input,
...
#pragma omp parallel for private(i) schedule(static)
    ...
    float_vect out = sg_derivative(in, window, order, delta);
